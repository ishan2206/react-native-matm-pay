...
class InstantpayMposModule(reactContext: ReactApplicationContext) : ReactContextBaseJavaModule(reactContext) {
      ...
      if (requestCode == 1) {
        ...
        when (resultCode) {
          ...
          CredopayPaymentConstants.TRANSACTION_COMPLETED -> {
            ...
            val res = JSONObject();
            if (data != null) {
              res.put("rrnm",data.getStringExtra("rrn"))
              res.put("tramsactionid",data.getStringExtra("transaction_id"))
              ...
              res.put("approvalcode",data.getStringExtra("approvalCode"))
              ...
              res.put("appVersion",data.getStringExtra("2.5"))
              ...
              res.put("tramsactiontype",data.getStringExtra("transaction_type"))
  ...
  fun startTransaction(options: String, prm: Promise){
    try {
      ...
      //Check Amount Key
      if(!items.has("amount") && getTransactionType!="BALANCE_ENQUIRY"){
